                            ...
                            <div class="aside-list-item js-list-item <?= $active_item_id == $u->get("id") ? "active" : "" ?>">
                                <div class="clearfix">
                                    ...
                                    <div class="inner">
                                        ...
                                        $datediffdays = abs(round($datediff / 86400));
                                        $datediffhours = abs(round($datediff / 3600));
                                        $datediffminutes = abs(round($datediff / 60)); //



                                        if ($datediff <= 20 and $datediff >= 0) {
                                            ?>
                                            <div class="sub"><span class="color-success">
                                            <span class="mdi mdi-check-circle"></span><?= $online = __("Online"); ?>  </span>
                                            </div>

                                            <?php
                                        } else {
                                            ?>
                                            <div class="sub"><span class="color-danger">
                                                            <span class="mdi mdi-close-circle"></span> <?= $online = __("Offline"); ?> </span>
                                            </div>
                                            <div class="sub"><span class="color-warn">

                                                    <?php if($datediffhours <=1){?>
                                                    <span class="mdi mdi-close-circle"></span> <?=   __("Last seen: ".$datediffminutes). " minute". ($datediffminutes >1 ? "s" : "").""; ?> </span>
                                                <?php } elseif($datediffdays <=1){?>
                                                    <span class="mdi mdi-close-circle"></span> <?=   __("Last seen: ".$datediffhours). " hour". ($datediffhours >1 ? "s" : "").""; ?> </span>
                                                <?php }elseif($datediffdays  > 100) {?>
                                                    <span class="mdi mdi-close-circle"></span> <?=   __("Last seen: Long time ago "); ?> </span>

                                                <?php } else {?>

                                                    <span class="mdi mdi-close-circle"></span> <?=   __("Last seen: ".$datediffdays). " day". ($datediffdays >1 ? "s" : "").""; ?> </span>
                                                    <?php
                                                }?>
                                            </div>
                                    ...
                                    <?php if ($AuthUser->canEdit($u)): ?>
                                        ...
                                        <a class="full-link js-ajaxload-content"
                                           href="<?= APPURL . "/users/" . $u->get("id") ?>"></a>
                                    <?php endif ?>
                                </div>
                            </div>
                        <?php endforeach ?>
