...
function install($Plugin)
{
    ...
    $sql  = "DROP TABLE IF EXISTS `".TABLE_PREFIX."auto_viewstory_schedule`;";
    $sql .= "CREATE TABLE `".TABLE_PREFIX."auto_viewstory_schedule` ( 
    ...
    $sql .= "DROP TABLE IF EXISTS `".TABLE_PREFIX."auto_viewstory_log`;";
    $sql .= "CREATE TABLE `".TABLE_PREFIX."auto_viewstory_log` ( 
                `id` INT NOT NULL AUTO_INCREMENT , 
                `user_id` INT NOT NULL , 
                `account_id` INT NOT NULL , 
                `status` VARCHAR(20) NOT NULL,
                `story_media_code` VARCHAR(50) NOT NULL,
                `data` TEXT NOT NULL , 
                `date` DATETIME NOT NULL , 
                PRIMARY KEY (`id`), 
                INDEX (`user_id`), 
                INDEX (`account_id`),
                INDEX (`story_media_code`)
            ) ENGINE = InnoDB;";

    $sql .= "ALTER TABLE `".TABLE_PREFIX."auto_viewstory_schedule` 
    ...
    $sql .= "ALTER TABLE `".TABLE_PREFIX."auto_viewstory_log` 
                ADD CONSTRAINT `".uniqid("ibfk_")."` FOREIGN KEY (`user_id`) 
                REFERENCES `".TABLE_PREFIX."users`(`id`) 
                ON DELETE CASCADE ON UPDATE CASCADE;";
...
function uninstall($Plugin)
{
    ...
    $sql = "DROP TABLE `".TABLE_PREFIX."auto_viewstory_schedule`;";
    $sql .= "DROP TABLE `".TABLE_PREFIX."auto_viewstory_log`;";
